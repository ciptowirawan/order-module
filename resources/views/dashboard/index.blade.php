...
<section style="background-color: #eee;">
    ...
    <div class="row">
        ...
        <div class="card mb-4">
            ...
            <h5 class="my-3 bold" style="color: dimgray">{{ $pendaftaran->full_name }}</h5>     
            ...
            <p class="badge badge-{{$pendaftaran->status == 'SUCCESS' ? 'success' : 'danger'}} fs-6">{{ $pendaftaran->status == 'SUCCESS' ? 'Active' : "Inactive" }}
            </div>
        </div>
        </div>
        ...
        <div class="col-lg-8">
            ...
            <div class="card-body">
                ...
                <div class="col-sm-8">
                    <p class="badge badge-{{$pendaftaran->payment->status == 'paid' ? 'success' : 'danger'}}">{{ $pendaftaran->payment->status }}</p>
                </div>
                </div>
                <hr> --}}
                <div class="row">
    ...
    {{-- <div class="my-2" align="center">
        @if($history > 0)
        <a href="/details/modify/{{$pendaftaran->id}}" class="btn btn-secondary font-weight-bold">See History</a>
        @endif
        <a href="/details/edit/{{$pendaftaran->id}}" class="btn mx-2 btn-primary font-weight-bold">Modify</a>
        <button type="button" class="btn btn-danger font-weight-bold" data-bs-toggle="modal" data-bs-target="#cancel{{ $pendaftaran->user->id }}">Cancel Registration</button> 
    </div> --}}

    <div class="modal hide fade in" tabindex="-1" id="cancel{{ $pendaftaran->id }}" role="dialog">
        <div class="modal-dialog" role="document">
            ...
            <form method="post" action="/details/destroy/{{ $pendaftaran->id }}">
                @csrf
                @method('delete')
